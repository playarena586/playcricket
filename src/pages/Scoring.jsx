...
export default function Scoring({ match, onExit }) {
  ...
  const bowler = bowling.players.find((player) => player.id === innings.bowler)?.name ?? 'Select bowler';
  ...
  const addBall = (runs, extras = 0, extraType = null) => {
    if (innings.completed || innings.pendingBatter) return;
    save(scoreBall(innings, { runs, extras, extraType, wicket: pendingWicket }));
    setPendingWicket(false);
  };
  const startSecondInnings = () => {
    const target = innings.runs + 1;
    const next = createInnings(bowling, batting, match.overs, target);
    setInningsList((current) => [...current, next]);
    setInningsIndex(1);
    setShowStats(false);
  };
  ...
  const result = matchComplete ? (innings.result === 'chased' ? `${batting.name} won by ${10 - innings.wickets} wickets` : innings.runs === inningsList[0].runs ? 'Match tied' : innings.runs > inningsList[0].runs ? `${batting.name} won` : `${bowling.name} won by ${inningsList[0].runs - innings.runs - 1} runs`) : null;
